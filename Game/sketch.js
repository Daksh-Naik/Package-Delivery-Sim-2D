...
function setup() {
    ...
    road = createSprite(400, height/2-15000, 500, 40000);
...
function draw() {
    ...
    if (gameState === Play) {
        ...
        if (keyDown("UP_ARROW") && frameCount % 70 === 0 && delvan.y < -15000 && delvan.y > -29000) {
            var roadworksimagesL3 = Math.round(random(1, 2));
            allRoadworks[2].remove();
            allRoadworks[3].remove();
